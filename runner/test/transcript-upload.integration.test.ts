...
describe("result capture transcript evidence", () => {
  ...
  it("sends the error and the evidence in one request, so a later failure cannot strand the anonymous half", async () => {
    ...
    // One request means the 503 is never reached. Emitting these separately
    // spends the 200 on the anonymous error and loses the named evidence to
    // the 503 — the receipt this change exists to stop producing.
    expect(platform.eventRequests()).toHaveLength(1);
    expect(platform.eventRequests()[0]).toEqual([
